...
internal sealed partial class WallpaperLayerEditorWindow : MyWindow
{
    ...
    // 检查器 TabStrip 分段与分组页（BuildInspectorTabStrip / ActivateInspectorPage /
    // SetPageVisibility / RefreshInspectorSegments / GroupSubtitle / SettingsRow）
    // 已按职责拆分到 WallpaperLayerEditor.Inspector.cs。
    ...
    // 检查器显隐同步、显示名映射、颜色解析与选项枚举（SliderControl / Selected /
    // DisplayModeName / DisplaySource / DisplayKind / ShapeTypeName / SmtcModeSuffix /
    // DisplayZOrder / Pick 及各项 Choices）已按职责拆分到 WallpaperLayerEditor.Inspector.cs。
